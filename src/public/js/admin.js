  ...
  $("#createNewPost").on("click", function (event) {
    ...
    $.ajax({
      ...
      success: function (data) {
        alert("Đã đăng một bài viết mới thành công");
        window.location.href = `${window.location.origin}/supporter/manage/posts`;
      },
      error: function (error) {
        alertify.error("Có lỗi gì đó xảy ra, vui lòng thử lại");
        console.log(error);
      },
    });
  });
...
function deleteClinicById() {
  $(".delete-specific-clinic").bind("click", function (e) {
    ...
    $.ajax({
      ...
      error: function (err) {
        alertify.error("Có lỗi gì đó đã xảy ra, vui lòng thử lại");
        console.log(err);
      },
    });
  });
        ...
        htmlNew += `
                <tr>
                    ...
                    <button type="button"  data-patient-id="${
                      patient.id
                    }" class="ml-3 btn btn-primary btn-new-patient-ok"> Receive</button>
                    <button  type="button" data-patient-id="${
                      patient.id
                    }" class="ml-3 btn btn-danger btn-new-patient-cancel"> Cancel </button>
                    </td>
                </tr>
                `;
      });
        ...
        htmlPending += `
                <tr>
                    ...
                    <button  data-patient-id="${
                      patient.id
                    }"  class="ml-3 btn btn-warning btn-pending-patient">Confirm</button>
                    <button  type="button" data-patient-id="${
                      patient.id
                    }" class="ml-3 btn btn-danger btn-pending-patient-cancel"> Cancel </button>
                    </td>
                </tr>
                `;
      });
      ...
      data.object.confirmedPatients.forEach((patient) => {
        htmlConfirmed += `
                <tr>
                    ...
                    <button  type="button" data-patient-id="${
                      patient.id
                    }"  class="ml-3 btn btn-info btn-confirmed-patient"> Information</button>
                    </td>
                </tr>
                `;
      });
      ...
      data.object.canceledPatients.forEach((patient) => {
        htmlCanceled += `
                <tr>
                    ...
                    <button   data-patient-id="${
                      patient.id
                    }"  class="ml-3 btn btn-primary btn-history-cancel-patient">History</button>
                    </td>
                </tr>
                `;
      });
...
function handleAfterCallingPatient() {
  $("#btn-confirm-patient-done").on("click", function (e) {
    if (
      !confirm(
        "Have you phoned to confirm whether the patient has an appointment?"
      )
    ) {
      return;
    }
...
function statisticalAdmin(month) {
  $.ajax({
    ...
    error: function (error) {
      alertify.error(
        "An error occurred while getting statistical information, please try again later"
      );
      console.log(error);
    },
  });
...
$(document).ready(function (e) {
  ...
  // });
  tinymce.init({
    selector: "#introEditDoctor, #description",
    height: 300,
    plugins:
