...
function getScheduleDoctorByDate() {
  $("#day-book").on("change", function (event) {
    ...
    $.ajax({
      ...
      data: { date: date, doctorId: doctorId },
      success: function (data) {
        // Xóa nội dung hiện tại bên trong div cha
        $("#div-schedule-id").html("");
        $("#div-more-info").html("");
        let html = "";
        let moreInfo = "";
        if (data.message.length > 0) {
          data.message.forEach((schedule, index) => {
            ...
            if (
              ...
              html += `<div>
                                  Hiện không có lịch khám trong khung thời gian này. Vui lòng chọn lịch khám tiếp theo.
                            </div>`;
            }
            ...
            moreInfo = `
                         <div class="d-flex flex-column">
                                    ...
                                    <span class="d-block pt-2">Giá khám: 300.000 VNĐ</span>
                                </div>
          ...
          html = `
                            <div>
                                 Bác sĩ "${data.doctor.name}" không có lịch hẹn vào ngày <b>${value}</b>. Vui lòng chọn lịch khám tiếp theo.
                            </div>
                    `;
          moreInfo = "";
        }
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      data.posts.rows.forEach((post) => {
        html += `
                            <a class="text-decoration-none" href="/detail/post/${
                              post.id
                            }">
  ...
  $.ajax({
    ...
    success: function (data) {
      if (typeof data.patient === "string") {
        alert(
          "Unfortunately, this appointment has enough patients booked, please choose a different time."
        );
...
function showModalBookingClinicPage() {
  $("#clinicRightContent").on(
    "click",
    ".show-modal-at-clinic-page",
    function () {
      let id = $(this).attr("id");
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $("#modal-avatar-doctor-spe").attr(
            "src",
            `/images/users/${data.doctor.avatar}`
          );
