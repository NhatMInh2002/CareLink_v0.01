  ...
  return new Promise(async (resolve, reject) => {
    try {
      let startDate = Date.parse(
        stringToDate(`01/${month}/2020`, "dd/MM/yyyy", "/")
      );
      let endDate = Date.parse(
        stringToDate(`31/${month}/2020`, "dd/MM/yyyy", "/")
      );
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!isCreatedBefore) {
        ...
        resolve(
          "Appointments are created successful (in 3 days). Please check your database (schedule table)"
        );
      } else {
        resolve(
          "Appointments are duplicated. Please check your database (schedule table)"
        );
      }
    } catch (e) {
      reject(e);
